...
Right now it's clear Ansible is the main driver. It has a self-hosted GitHub Actions runner, GitHub Actions calls an Ansible playbook on deployment, the very same playbook used when first configuring the web server, and is very much the hub of my entire CI/CD pipeline. Really the only thing that GitHub Actions is providing is building and publishing of the API itself, but even that could have been done in Ansible. 

But containerising the app has meant there's a real shift to making GitHub Actions my hub for CI/CD. Calling a basic Ansible playbook for CD will still happen, yeah, but now that's only for my homelab. There's a real possibility for me to also deploy to Azure or AWS and be able to run containers from the very image that my GitHub Actions flow created. And that feels huge, like a real modern shift. I've essentially gone from homelab only to a train of thought being opened that involves Cloudflare load balancing and a hybrid solution. 
